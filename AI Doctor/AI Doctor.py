...
diets=pd.read_csv('Diets_ds.csv')
precautions_df=pd.read_csv('precautions_ds.csv')
workout_df=pd.read_csv('workout_ds.csv')
...
def prediction():
    ...
    for s in user_symptoms:
        if s in symptoms:
            index=symptoms[s]
            have_symptoms[index]=1
    have_symptoms=np.array(have_symptoms).reshape(1,-1)
    predicted=model.predict(have_symptoms)
    bemari=LE.inverse_transform(predicted)
    diets_=diets.loc[diets['Disease']==bemari[0],'Diet'].values
    workout_df_=workout_df.loc[workout_df['disease']==bemari[0],'workout'].values
    precautions_df_=precautions_df.loc[precautions_df['Disease']==bemari[0],['Precaution_1','Precaution_2','Precaution_3','Precaution_4']].values
    diet_list = eval(diets_[0]) if diets_ else []
    workout_list=workout_df_
    precautions_list=precautions_df_[0]
